...
async function saveAction(formData: FormData) {
  "use server";
  await createOrUpdateAdminItem("role", {
...
export default async function Page() {
  const items = await listAdminItems("role");

  return (
    <AdminShell title="Roles & Permissions" description="Add, update and delete role records for admin planning.">
      <form action={saveAction} className="grid gap-4 rounded-2xl bg-[#F3EFE6] p-6 md:grid-cols-2">
        ...
        <button className="rounded bg-[#111111] px-5 py-3 font-semibold text-white md:col-span-2">
          Add or Update Roles & Permissions
        </button>
      </form>
