...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    if (product) {
      const nextItem: CartItem = {
        id: product,
        name: params.get("name") || "Dadur Bari Product",
        category: params.get("category") || "Dadur Bari",
        size: params.get("size") || "L",
      ...
      if (found) {
        current = current.map((item) =>
          item.id === nextItem.id && item.size === nextItem.size
  ...
  function setQuantity(id: string, size: string, quantity: number) {
  ...
  if (!loaded) {
    return <main className="min-h-screen bg-[#F3EFE6] p-10">Loading...</main>;
  }
    ...
    <main className="min-h-screen bg-[#F3EFE6] text-[#111111]">
      ...
      <section className="bg-[#111111] py-16 text-white">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <p className="mt-4 text-white/70">Review your selected products.</p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-[1fr_360px]">
        <div className="space-y-5">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={`${item.id}-${item.size}`} className="rounded-3xl bg-white p-6 shadow-lg">
                <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
                  <div>
                    <h2 className="text-2xl font-bold">{item.name}</h2>
                    <p className="mt-2 text-black/60">{item.category} / Size {item.size}</p>
                    <div className="mt-4 flex items-center gap-3">
                      <button type="button" onClick={() => setQuantity(item.id, item.size, item.quantity - 1)} className="rounded border px-4 py-2 font-bold">-</button>
                      <span className="min-w-8 text-center font-bold">{item.quantity}</span>
                      <button type="button" onClick={() => setQuantity(item.id, item.size, item.quantity + 1)} className="rounded border px-4 py-2 font-bold">+</button>
                    </div>
                  </div>
                  <div className="text-left md:text-right">
                    <p className="text-sm text-black/50">Unit: ৳{item.price}</p>
                    <p className="text-2xl font-bold">৳{item.price * item.quantity}</p>
                    <button type="button" onClick={() => removeItem(item.id, item.size)} className="mt-3 text-sm font-semibold text-red-600">Remove</button>
                  </div>
                </div>
              </div>
            ))
        ...
        <aside className="h-fit rounded-3xl bg-white p-6 shadow-lg">
          ...
          <div className="mt-6 space-y-4 text-sm">
            ...
            <div className="border-t pt-4 flex justify-between text-xl font-bold"><span>Total</span><span>৳{total}</span></div>
          </div>

          {items.length > 0 && (
            <Link href="/checkout" className="mt-6 block rounded-md bg-[#111111] px-6 py-4 text-center font-semibold text-white">
              Proceed to Checkout
            </Link>
          )}

          <Link href="/shop" className="mt-3 block rounded-md border border-[#111111] px-6 py-4 text-center font-semibold">
            Continue Shopping
          </Link>
        </aside>
      </section>
